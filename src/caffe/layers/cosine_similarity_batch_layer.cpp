...
void CosineSimilarityBatchLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    for (int j = 0; j < bottom[0]->num(); ++j) { //optimize
  ...
  int k = 0;
  for (int i = 0; i < bottom[0]->num(); ++i) {
    //LOG(INFO) << "bottom[1]->cpu_data()[i] " << bottom[1]->cpu_data()[i];
...
void CosineSimilarityBatchLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < num; ++i) {
	...
	//i j --> h    
        ...
        Dtype add_dsdx = top[0]->cpu_diff()[h] * (bottom[0]->cpu_data()[j*channels + k]/sqrt(xy_[i][i] * xy_[j][j]) - xy_[i][j]/xy_[i][i] * bottom[0]->cpu_data()[i*channels + k]/sqrt(xy_[i][i] * xy_[j][j]));
        dsdx += add_dsdx;
    }		
