...
template <typename Dtype>
class BatchPairingLayer : public Layer<Dtype>{
 public:
  explicit BatchPairingLayer(const LayerParameter& param)
      : Layer<Dtype>(param){}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "BatchPairingLayer"; }
...
class CosineSimilarityLayer : public Layer<Dtype>{
 public:
  explicit CosineSimilarityLayer(const LayerParameter& param)
      : Layer<Dtype>(param), xx_(), xy_(), yy_(), summer_vec_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top); 
...
class EntropyLossLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  Blob<Dtype> prob_;
  /// entropy stores the entropies computed along chosen softmax axis.
  Blob<Dtype> entropy_;
...
class SoftmaxWithLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  vector<Dtype> classes_normalization_;
  bool normalize_classes_; // proto parameter
