...
template <typename Dtype>
class ScalerLayer : public NeuronLayer<Dtype> {
 public:
  explicit ScalerLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "Scaler"; }
 ...
 protected:
  ...
  Dtype lower_bound_, upper_bound_, alpha_, max_iter_, coeff_;
};
