...
class LMDBLabelCursor : public LMDBCursor {
 public:
  ...
  virtual ~LMDBLabelCursor() {}

  virtual void SeekToFirst() { 
    std::random_shuffle (labels_.begin(), labels_.end());
    labels_iter_ = labels_.begin();
    keys_iter_ = labels_keys_[*labels_iter_].begin();
    mdb_key_ = *keys_iter_;
    curr_inlabel_samples = 0;
    Seek(MDB_SET_KEY);
  }

  virtual void Next() { 
    if ((keys_iter_ != labels_keys_[*labels_iter_].end()) && (curr_inlabel_samples < inlabel_samples)){
      //std::cout << "*labels_iter_ "<< *labels_iter_ << std::endl;
      keys_iter_++;
      curr_inlabel_samples++;
      
    }
    if ((labels_iter_ != labels_.end()) &&( (keys_iter_ == labels_keys_[*labels_iter_].end()) || (curr_inlabel_samples == inlabel_samples))){
      //LOG(INFO) << "curr_inlabel_samples"  << curr_inlabel_samples <<  " "<< *labels_iter_ ;
      
      std::random_shuffle (labels_keys_[*labels_iter_].begin(), labels_keys_[*labels_iter_].end());
      curr_inlabel_samples = 0;
      
      labels_iter_++;
      keys_iter_ = labels_keys_[*labels_iter_].begin();	
    }
    if (labels_iter_ == labels_.end()) {
      valid_ = false;
    } else {
      mdb_key_ = *keys_iter_;
      Seek(MDB_SET_KEY);
 ...
 private:
  ...
  map<int, vector<MDB_val> > labels_keys_;
  vector<int> labels_;
  vector<int>::iterator labels_iter_;
  vector<MDB_val>::iterator keys_iter_; 
  int inlabel_samples, curr_inlabel_samples;
};
